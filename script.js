...
// ==========================================================
// MODO ECONÔMICO SUPABASE
// Reduz Egress/Saída, leituras repetidas, consumo de Storage e tráfego de mídia.
// ==========================================================
const ECONOMIA_SUPABASE_ATIVA = true;
const CONVERSAS_POR_CARGA = 350;
const MENSAGENS_POR_CONVERSA = 90;
const NOTAS_POR_CONVERSA = 90;
const POLLING_ECONOMICO_MS = 60 * 1000;
const DEBOUNCE_REALTIME_CONVERSAS_MS = 650;
const DEBOUNCE_REALTIME_MENSAGENS_MS = 450;

const CONVERSAS_SELECT = 'id,cliente_id,departamento_id,atendente_id,etiqueta_id,status,ultima_mensagem,ultima_mensagem_tipo,ultima_mensagem_em,criado_em,atualizado_em,evolution_remote_jid';
const CLIENTES_SELECT = 'id,nome,telefone,foto_url,tipo_cliente,ativo,criado_em';
const MENSAGENS_SELECT = 'id,conversa_id,remetente_tipo,remetente_nome,texto,tipo,media_url,media_nome,evolution_message_id,evolution_payload,criado_em';
const NOTAS_SELECT = 'id,conversa_id,usuario_id,nota,criado_em';

const MEDIA_LIMITES_BYTES = {
  IMAGEM: 50 * 1024 * 1024,
  AUDIO: 15 * 1024 * 1024,
  VIDEO: 35 * 1024 * 1024,
  DOCUMENTO: 25 * 1024 * 1024,
};
const IMAGEM_MAX_LADO = 1600;
const IMAGEM_QUALIDADE = 0.72;
const IMAGEM_COMPRIMIR_ACIMA_DE = 450 * 1024;

// Monitor local do Supabase: não faz SELECT novo, não chama Storage e não usa API administrativa.
// Ele usa somente estimativas do que o front-end já trafegou e valores manuais informados pelo usuário.
const MONITOR_SUPABASE_STORAGE_KEY = 'medplus_supabase_monitor_local_v1';
const MONITOR_GB = 1024 * 1024 * 1024;
const MONITOR_MB = 1024 * 1024;
const MONITOR_LIMITES_PADRAO = {
  egressBytes: 5 * MONITOR_GB,
  dbBytes: 500 * MONITOR_MB,
  mau: 50000,
  storageBytes: 1 * MONITOR_GB
};
const MONITOR_MEDIA_DOWNLOAD_ESTIMADO = {
  IMAGEM: 350 * 1024,
  AUDIO: 1.2 * MONITOR_MB,
  VIDEO: 6 * MONITOR_MB,
  DOCUMENTO: 1 * MONITOR_MB
};
...
let refreshConversasTimer = null;
let refreshMensagensTimer = null;
let realtimeConectado = false;
let historicoAtualLimitado = false;
...
let notaQuickReplyIndex = 0;
let notaQuickReplyContext = null;
let notasInternasNotificadas = new Set();
let monitorSupabaseDados = null;
let monitorSupabaseTimer = null;
...
function coletarAtendentesParaMencao() {
  const mapa = new Map();

  function adicionar(usuario, origem = '') {
    if (!usuario) return;

    const nome = String(usuario.nome || usuario.name || '').trim();
    const email = String(usuario.email || '').trim();
    const perfil = String(usuario.perfil || usuario.tipo || usuario.role || 'Atendente').trim();
    const id = String(usuario.id || usuario.usuario_id || email || nome || '').trim();
    ...
    mapa.set(id, {
      ...
      ...usuario,
      id,
      nome: nome || existente.nome || email || 'Atendente',
      email: email || existente.email || '',
      perfil: perfil || existente.perfil || 'Atendente',
      _origem_mencao: origem || existente._origem_mencao || 'usuarios'
    });
  }

  // Fonte principal: tabela/array de usuários carregada pelo sistema.
  (usuarios || []).forEach((u) => adicionar(u, 'usuarios'));

  // Segurança visual: se alguma conversa já trouxe o atendente preenchido,
  // também entra na lista de menção. Isso evita perder nomes por filtros/RLS.
  (todasConversas || []).forEach((c) => adicionar(c.atendente, 'conversa'));

  // Fallback pela interface: aproveita opções já carregadas nos selects.
  // Útil quando o layout/app é carregado antes de algum array ser atualizado.
  document.querySelectorAll('#atendenteFilter option, #transferAtendenteSelect option').forEach((option) => {
    const valor = String(option.value || '').trim();
    const texto = String(option.textContent || '').trim();
    if (!valor || !texto || /^todos|selecione/i.test(texto)) return;
  ...
  // Mantém o usuário logado como opção, mas não deixa ele apagar os demais.
  adicionar(usuarioAtual, 'usuarioAtual');

  return [...mapa.values()]
    .filter(usuarioEhAtendente)
    .sort((a, b) => nomeUsuario(a).localeCompare(nomeUsuario(b), 'pt-BR', { sensitivity: 'base' }));
}

function atendentesMencaoFiltrados(termo = '') {
  const busca = normalizarMencao(termo);
  return coletarAtendentesParaMencao()
    .filter((u) => {
...
function configurarMencoesNotaInterna() {
  ...
  input.addEventListener('input', () => {
    notaMentionIndex = 0;
    notaQuickReplyIndex = 0;
    atualizarSugestoesNotaInterna();
  });

  input.addEventListener('click', atualizarSugestoesNotaInterna);
  ...
  input.addEventListener('keydown', (e) => {
    const quickNotaAberto = !$('notaQuickRepliesBox')?.classList.contains('hidden');

    if (quickNotaAberto && e.key === 'ArrowDown') {
      e.preventDefault();
      moverSelecaoMensagemRapidaNota(1);
      return;
    }

    if (quickNotaAberto && e.key === 'ArrowUp') {
      e.preventDefault();
      moverSelecaoMensagemRapidaNota(-1);
      return;
    }

    if (quickNotaAberto && ((e.key === 'Enter' && !e.shiftKey) || e.key === 'Tab')) {
      e.preventDefault();
      if (selecionarMensagemRapidaNotaAtual()) return;
    }

    if (quickNotaAberto && e.key === 'Escape') {
      e.preventDefault();
      esconderMensagensRapidasNota();
      return;
    }
...
function mesAtualMonitorSupabase() {
  return new Date().toISOString().slice(0, 7);
}

function criarMonitorSupabasePadrao() {
  return {
    mes: mesAtualMonitorSupabase(),
    egressBytes: 0,
    dbEstimadoBytes: 0,
    storageEstimadoBytes: 0,
    mauUsuarios: {},
    limites: { ...MONITOR_LIMITES_PADRAO },
    manual: {
      egressBytes: null,
      dbBytes: null,
      mau: null,
...
function carregarMonitorSupabaseLocal() {
  if (monitorSupabaseDados) return monitorSupabaseDados;
  try {
    const salvo = JSON.parse(localStorage.getItem(MONITOR_SUPABASE_STORAGE_KEY) || 'null');
    monitorSupabaseDados = salvo && typeof salvo === 'object' ? salvo : criarMonitorSupabasePadrao();
  } catch (_) {
    monitorSupabaseDados = criarMonitorSupabasePadrao();
  }

  const mes = mesAtualMonitorSupabase();
  if (monitorSupabaseDados.mes !== mes) {
    monitorSupabaseDados.mes = mes;
    monitorSupabaseDados.egressBytes = 0;
    monitorSupabaseDados.mauUsuarios = {};
    monitorSupabaseDados.atualizadoEm = new Date().toISOString();
  }

  monitorSupabaseDados.limites = {
    ...MONITOR_LIMITES_PADRAO,
    ...(monitorSupabaseDados.limites || {})
  };
  monitorSupabaseDados.manual = {
    egressBytes: null,
    dbBytes: null,
    mau: null,
    storageBytes: null,
  ...
  monitorSupabaseDados.mauUsuarios = monitorSupabaseDados.mauUsuarios || {};
  return monitorSupabaseDados;
}

function salvarMonitorSupabaseLocal() {
  const dados = carregarMonitorSupabaseLocal();
  dados.atualizadoEm = new Date().toISOString();
  try {
    localStorage.setItem(MONITOR_SUPABASE_STORAGE_KEY, JSON.stringify(dados));
  } catch (_) {}
  renderizarMonitorSupabase();
}

function tamanhoJsonEstimadoBytes(valor) {
  try {
    return new Blob([JSON.stringify(valor || null)]).size;
  } catch (_) {
    try { return JSON.stringify(valor || null).length; } catch { return 0; }
  }
}

function registrarMonitorPayloadRecebido(payload, fator = 1) {
  if (!ECONOMIA_SUPABASE_ATIVA) return;
  const dados = carregarMonitorSupabaseLocal();
  const bytes = Math.max(0, tamanhoJsonEstimadoBytes(payload) * Number(fator || 1));
  if (!bytes) return;
  dados.egressBytes = Number(dados.egressBytes || 0) + bytes;
  dados.dbEstimadoBytes = Math.max(Number(dados.dbEstimadoBytes || 0), Math.round(bytes * 1.35));
  salvarMonitorSupabaseLocal();
}

function registrarMonitorUpload(file, arquivoOriginal = null) {
  if (!file) return;
  const dados = carregarMonitorSupabaseLocal();
  dados.storageEstimadoBytes = Number(dados.storageEstimadoBytes || 0) + Number(file.size || 0);
  if (arquivoOriginal && arquivoOriginal.size && file.size < arquivoOriginal.size) {
    dados.storageEconomizadoBytes = Number(dados.storageEconomizadoBytes || 0) + (Number(arquivoOriginal.size) - Number(file.size));
  }
  salvarMonitorSupabaseLocal();
}

function registrarMonitorMidiaSobDemanda(tipo) {
  const dados = carregarMonitorSupabaseLocal();
  const chave = String(tipo || 'DOCUMENTO').toUpperCase();
  dados.egressBytes = Number(dados.egressBytes || 0) + (MONITOR_MEDIA_DOWNLOAD_ESTIMADO[chave] || MONITOR_MEDIA_DOWNLOAD_ESTIMADO.DOCUMENTO);
  salvarMonitorSupabaseLocal();
}

function registrarUsuarioMonitorSupabase() {
  if (!usuarioAtual) return;
  const dados = carregarMonitorSupabaseLocal();
  const chave = String(usuarioAtual.email || usuarioAtual.id || 'usuario-local').toLowerCase();
  dados.mauUsuarios[chave] = true;
  salvarMonitorSupabaseLocal();
}

function formatarBytesMonitor(bytes) {
  const n = Number(bytes || 0);
  if (n >= MONITOR_GB) return `${(n / MONITOR_GB).toFixed(n >= 10 * MONITOR_GB ? 0 : 2)} GB`;
  if (n >= MONITOR_MB) return `${(n / MONITOR_MB).toFixed(n >= 10 * MONITOR_MB ? 0 : 1)} MB`;
  if (n >= 1024) return `${Math.round(n / 1024)} KB`;
  return `${Math.round(n)} B`;
}

function numeroMonitorFormatado(n) {
  return Number(n || 0).toLocaleString('pt-BR');
}

function valorMonitorAtual(chave) {
  const dados = carregarMonitorSupabaseLocal();
  if (chave === 'egress') return dados.manual.egressBytes ?? Number(dados.egressBytes || 0);
  if (chave === 'db') return dados.manual.dbBytes ?? Number(dados.dbEstimadoBytes || 0);
  if (chave === 'mau') return dados.manual.mau ?? Object.keys(dados.mauUsuarios || {}).length;
  if (chave === 'storage') return dados.manual.storageBytes ?? Number(dados.storageEstimadoBytes || 0);
  return 0;
}

function percentualMonitor(usado, limite) {
  const pct = limite > 0 ? (Number(usado || 0) / Number(limite || 1)) * 100 : 0;
  return Math.max(0, Math.min(100, pct));
}

function aplicarBarraMonitor(prefixo, usado, limite, tipo = 'bytes') {
  const pct = percentualMonitor(usado, limite);
  const bar = $(`monitor${prefixo}Bar`);
  const percent = $(`monitor${prefixo}Percent`);
  const value = $(`monitor${prefixo}Value`);
  const limit = $(`monitor${prefixo}Limit`);

  if (bar) {
    bar.style.width = `${pct.toFixed(1)}%`;
    bar.classList.toggle('monitor-warning', pct >= 70 && pct < 90);
    bar.classList.toggle('monitor-danger', pct >= 90);
  }
  if (percent) percent.textContent = `${pct.toFixed(0)}%`;
  if (value) value.textContent = tipo === 'numero' ? `${numeroMonitorFormatado(usado)} usuário${Number(usado) === 1 ? '' : 's'}` : formatarBytesMonitor(usado);
  if (limit) limit.textContent = tipo === 'numero' ? numeroMonitorFormatado(limite) : formatarBytesMonitor(limite);
}

function renderizarMonitorSupabase() {
  const painel = $('monitorDadosPanel');
  if (!painel) return;

  const dados = carregarMonitorSupabaseLocal();
  aplicarBarraMonitor('Egress', valorMonitorAtual('egress'), dados.limites.egressBytes, 'bytes');
  aplicarBarraMonitor('Db', valorMonitorAtual('db'), dados.limites.dbBytes, 'bytes');
  aplicarBarraMonitor('Mau', valorMonitorAtual('mau'), dados.limites.mau, 'numero');
  aplicarBarraMonitor('Storage', valorMonitorAtual('storage'), dados.limites.storageBytes, 'bytes');

  const resumo = $('monitorStatusResumo');
  if (resumo) {
    resumo.textContent = `Local • ${dados.mes} • sem consulta extra`;
  }

  const mapMb = (bytes) => bytes == null ? '' : Math.round(Number(bytes || 0) / MONITOR_MB);
  if ($('monitorManualEgressMb')) $('monitorManualEgressMb').value = dados.manual.egressBytes == null ? '' : mapMb(dados.manual.egressBytes);
  if ($('monitorManualDbMb')) $('monitorManualDbMb').value = dados.manual.dbBytes == null ? '' : mapMb(dados.manual.dbBytes);
  if ($('monitorManualMau')) $('monitorManualMau').value = dados.manual.mau == null ? '' : dados.manual.mau;
  if ($('monitorManualStorageMb')) $('monitorManualStorageMb').value = dados.manual.storageBytes == null ? '' : mapMb(dados.manual.storageBytes);

  if ($('monitorLimitEgressMb')) $('monitorLimitEgressMb').value = mapMb(dados.limites.egressBytes);
  if ($('monitorLimitDbMb')) $('monitorLimitDbMb').value = mapMb(dados.limites.dbBytes);
  if ($('monitorLimitMau')) $('monitorLimitMau').value = dados.limites.mau;
  if ($('monitorLimitStorageMb')) $('monitorLimitStorageMb').value = mapMb(dados.limites.storageBytes);
}

function alternarMonitorDados() {
  const painel = $('monitorDadosPanel');
  const btn = $('btnToggleMonitorDados');
  ...
  if (abrir) renderizarMonitorSupabase();
}

function numeroInputMonitor(id) {
  const el = $(id);
...
function salvarConfiguracaoMonitorSupabase() {
  const dados = carregarMonitorSupabaseLocal();
  dados.manual.egressBytes = numeroInputMonitor('monitorManualEgressMb') == null ? null : numeroInputMonitor('monitorManualEgressMb') * MONITOR_MB;
  dados.manual.dbBytes = numeroInputMonitor('monitorManualDbMb') == null ? null : numeroInputMonitor('monitorManualDbMb') * MONITOR_MB;
  dados.manual.mau = numeroInputMonitor('monitorManualMau');
  dados.manual.storageBytes = numeroInputMonitor('monitorManualStorageMb') == null ? null : numeroInputMonitor('monitorManualStorageMb') * MONITOR_MB;

  dados.limites.egressBytes = Math.max(MONITOR_MB, Number(numeroInputMonitor('monitorLimitEgressMb') || (MONITOR_LIMITES_PADRAO.egressBytes / MONITOR_MB)) * MONITOR_MB);
  dados.limites.dbBytes = Math.max(MONITOR_MB, Number(numeroInputMonitor('monitorLimitDbMb') || (MONITOR_LIMITES_PADRAO.dbBytes / MONITOR_MB)) * MONITOR_MB);
  dados.limites.mau = Math.max(1, Number(numeroInputMonitor('monitorLimitMau') || MONITOR_LIMITES_PADRAO.mau));
  dados.limites.storageBytes = Math.max(MONITOR_MB, Number(numeroInputMonitor('monitorLimitStorageMb') || (MONITOR_LIMITES_PADRAO.storageBytes / MONITOR_MB)) * MONITOR_MB);

  salvarMonitorSupabaseLocal();
  toast('Monitor de dados atualizado sem consultar o Supabase.');
}

function limparValoresManuaisMonitorSupabase() {
  const dados = carregarMonitorSupabaseLocal();
  dados.manual = { egressBytes: null, dbBytes: null, mau: null, storageBytes: null };
  salvarMonitorSupabaseLocal();
  toast('Valores manuais limpos. O painel voltou para estimativas locais.');
}

function resetarMonitorSupabase() {
  if (!confirm('Zerar apenas as estimativas locais deste navegador? Isso não altera nada no Supabase.')) return;
  monitorSupabaseDados = criarMonitorSupabasePadrao();
  salvarMonitorSupabaseLocal();
  toast('Estimativas locais zeradas.');
}

function iniciarMonitorSupabase() {
  carregarMonitorSupabaseLocal();
  registrarUsuarioMonitorSupabase();
  renderizarMonitorSupabase();
  if (monitorSupabaseTimer) clearInterval(monitorSupabaseTimer);
  monitorSupabaseTimer = setInterval(renderizarMonitorSupabase, 30000);
}

async function carregarDadosBase() {
  const [depsRes, etiqRes, usersRes, userDepsRes] = await Promise.all([
    db.from('departamentos').select('id,nome,descricao,ativo').order('nome', { ascending: true }),
    db.from('etiquetas').select('id,nome,cor,ativo').order('nome', { ascending: true }),
    db.from('usuarios').select('id,nome,email,perfil,ativo').order('nome', { ascending: true }),
    db.from('usuario_departamentos').select('id,usuario_id,departamento_id,ativo')
  ]);
  ...
  registrarMonitorPayloadRecebido([depsRes.data, etiqRes.data, usersRes.data, userDepsRes.data]);
...
async function carregarMensagensRapidas(mostrarErro = false) {
  ...
  try {
    ...
    registrarMonitorPayloadRecebido(data);
...
async function carregarConversas(opcoes = {}) {
  ...
  registrarMonitorPayloadRecebido(lista);
  ...
  const clientesMap = await buscarTabelaPorIds('clientes', lista.map((c) => c.cliente_id), CLIENTES_SELECT);
...
function renderizarConversas() {
  ...
  if (filtradas.length) {
    html += filtradas.map((c) => {
      ...
      const resumoUltima = resumoUltimaMensagemConversa(c);
      ...
      return `
        <button class="conversation-card ${ativo} ${destaque} ${unreadClass}" data-id="${escapeHtml(c.id)}" type="button">
          ...
          <div class="card-main">
            ...
            <div class="card-last">${escapeHtml(resumoUltima)}</div>
            <div class="card-badges">
...
function renderizarKanban() {
  ...
  board.innerHTML = KANBAN_STATUS.map((status) => {
    ...
    const cards = itens.length ? itens.map((c) => {
      ...
      const resumoUltima = resumoUltimaMensagemConversa(c) || 'Sem última mensagem';
      const statusMetaCard = statusKanbanMeta(c.status);

      return `
        <button class="kanban-client-card ${statusMetaCard.classe} ${ativo} ${destaque} ${unreadClass}" draggable="true" data-id="${escapeHtml(c.id)}" data-status="${escapeHtml(statusMetaCard.id)}" type="button" title="Clique para abrir o chat">
          ...
          <div class="kanban-client-last">${escapeHtml(resumoUltima)}</div>
...
async function carregarMensagensENotas(conversaId, opcoes = {}) {
  ...
  const [msgRes, notasRes] = await Promise.all([
    db.from('mensagens')
      .select(MENSAGENS_SELECT)
      .eq('conversa_id', conversaId)
      .order('criado_em', { ascending: false })
      .limit(MENSAGENS_POR_CONVERSA),
    db.from('notas_internas')
      .select(NOTAS_SELECT)
      .eq('conversa_id', conversaId)
      .order('criado_em', { ascending: false })
      .limit(NOTAS_POR_CONVERSA)
  ]);

  registrarMonitorPayloadRecebido([msgRes.data, notasRes.data]);
  ...
  historicoAtualLimitado = (msgRes.data || []).length >= MENSAGENS_POR_CONVERSA || (notasRes.data || []).length >= NOTAS_POR_CONVERSA;
...
function renderizarMensagens(mensagens) {
  ...
  const avisoHistorico = historicoAtualLimitado
    ? '<div class="history-economy-notice">Modo econômico: exibindo as mensagens mais recentes desta conversa para reduzir Egress do Supabase.</div>'
    : '';
  box.innerHTML = avisoHistorico + mensagens.map(renderizarMensagem).join('');
  ...
  document.querySelectorAll('[data-load-media]').forEach((el) => {
    el.addEventListener('click', () => carregarMidiaSobDemanda(el));
  });

  document.querySelectorAll('[data-copy-contact]').forEach((el) => {
    el.addEventListener('click', (ev) => {
      ev.preventDefault();
      ev.stopPropagation();
      copiarContatoRecebido(el);
    });
  });
...
function ehPlaceholderContato(texto = '', tipo = '') {
  ...
  return [
    ...
    'contato',
    'contatos'
  ].includes(t) || k.includes('contact') || k.includes('contato');
}

function decodificarValorVcard(valor = '') {
  return String(valor || '')
    .replace(/\\n/g, '\n')
    .replace(/\\,/g, ',')
    .replace(/\\;/g, ';')
...
function buscarCampoVcard(vcard = '', campo = '') {
  const alvo = String(campo || '').toUpperCase();
  if (!vcard || !alvo) return '';

  const linhas = String(vcard)
    .replace(/\r/g, '')
    .split('\n')
  ...
  for (const linha of linhas) {
    ...
    const chave = linha.slice(0, idx).toUpperCase();
    const valor = decodificarValorVcard(linha.slice(idx + 1));

    if (
      chave === alvo ||
      chave.endsWith(`.${alvo}`) ||
      chave.startsWith(`${alvo};`) ||
      chave.includes(`.${alvo};`)
    ) {
      return valor;
    }
...
function buscarTelefoneVcard(vcard = '') {
  if (!vcard) return '';
  ...
  for (const linha of linhas) {
    const idx = linha.indexOf(':');
    const chave = idx >= 0 ? linha.slice(0, idx) : linha;
    const valor = idx >= 0 ? linha.slice(idx + 1) : '';

    const waid = chave.match(/waid=(\d+)/i);
    if (waid?.[1]) return waid[1];

    if (/TEL/i.test(chave) && valor) return decodificarValorVcard(valor);
  }

  const waidSolto = String(vcard).match(/waid=(\d+)/i);
  if (waidSolto?.[1]) return waidSolto[1];

  return '';
}

function limparTelefoneContato(valor = '') {
  return String(valor || '')
...
function telefoneContatoParaWaMe(telefone = '') {
  const digitos = String(telefone || '').replace(/\D/g, '');
  return digitos ? `https://wa.me/${digitos}` : '';
}

function normalizarContatoCompartilhado(origem, fallbackNome = '') {
  if (!origem) return null;

  if (typeof origem === 'string') {
    const vcardTexto = origem.includes('BEGIN:VCARD') ? origem : '';
    if (!vcardTexto) return null;
    return normalizarContatoCompartilhado({ vcard: vcardTexto }, fallbackNome);
  }
  ...
  let nome = String(
    ...
    origem.name ||
    origem.fullName ||
    origem.formattedName ||
    origem.pushName ||
    fallbackNome ||
    buscarCampoVcard(vcard, 'FN') ||
    buscarCampoVcard(vcard, 'N') ||
    ''
  ).trim();
  ...
  const organizacao = String(
    origem.organization ||
    origem.org ||
    buscarCampoVcard(vcard, 'ORG') ||
    ''
  ).trim();

  telefone = limparTelefoneContato(telefone);

  if (!nome && telefone) nome = 'Contato compartilhado';
  if (!nome && !telefone && !vcard) return null;

  return {
    nome: nome || 'Contato compartilhado',
    telefone,
    organizacao,
    vcard
  };
...
function adicionarContatoSeValido(lista, contato) {
  if (!contato) return;
  const chave = `${normalizarMencao(contato.nome)}|${limparTelefoneContato(contato.telefone)}|${String(contato.vcard || '').slice(0, 80)}`;
  if (lista.some((item) => item._chave === chave)) return;
  lista.push({ ...contato, _chave: chave });
}

function coletarContatosDoObjeto(obj, lista, profundidade = 0) {
  if (!obj || profundidade > 7) return;

  if (Array.isArray(obj)) {
    obj.forEach((item) => coletarContatosDoObjeto(item, lista, profundidade + 1));
    return;
  }
  ...
  Object.entries(obj).forEach(([chave, valor]) => {
    ...
    if (k === 'contactmessage') {
      adicionarContatoSeValido(lista, normalizarContatoCompartilhado(valor));
      return;
    }

    if (k === 'contactsarraymessage') {
      const arr = valor || {};
      const contatos = arr.contacts || arr.contact || arr.vcards || arr.cards || [];
      if (Array.isArray(contatos)) {
        contatos.forEach((contato) => adicionarContatoSeValido(lista, normalizarContatoCompartilhado(contato, arr.displayName)));
      } else {
        adicionarContatoSeValido(lista, normalizarContatoCompartilhado(contatos, arr.displayName));
      }
      return;
    }

    if (Array.isArray(valor) || (valor && typeof valor === 'object')) {
      coletarContatosDoObjeto(valor, lista, profundidade + 1);
    }
  });
}

function extrairContatosMensagem(m) {
  if (Array.isArray(m?._contatosExtraidos)) return m._contatosExtraidos;

  const contatos = [];
  const payload = parsePayloadMensagem(m?.evolution_payload) || {};
  const textoJson = parsePayloadMensagem(m?.texto) || null;

  coletarContatosDoObjeto(payload, contatos);
  if (textoJson) coletarContatosDoObjeto(textoJson, contatos);

  const fontesDiretas = [
    payload?.contactMessage,
    payload?.contactsArrayMessage,
    payload?.message?.contactMessage,
    payload?.message?.contactsArrayMessage,
    payload?.data?.message?.contactMessage,
    payload?.data?.message?.contactsArrayMessage,
    payload?.data?.message?.message?.contactMessage,
    payload?.data?.message?.message?.contactsArrayMessage,
    payload?.data?.message?.message?.message?.contactMessage,
    payload?.data?.message?.message?.message?.contactsArrayMessage,
    payload?.message?.message?.contactMessage,
    payload?.message?.message?.contactsArrayMessage,
  ];

  fontesDiretas.forEach((fonte) => {
    if (!fonte) return;
    if (fonte.contacts && Array.isArray(fonte.contacts)) {
      fonte.contacts.forEach((contato) => adicionarContatoSeValido(contatos, normalizarContatoCompartilhado(contato, fonte.displayName)));
    } else {
      adicionarContatoSeValido(contatos, normalizarContatoCompartilhado(fonte));
    }
  });

  const limpos = contatos.map(({ _chave, ...contato }) => contato);
  if (m) m._contatosExtraidos = limpos;
  return limpos;
}

function mensagemTemContato(m) {
  return extrairContatosMensagem(m).length > 0 || ehPlaceholderContato(m?.texto, m?.tipo);
}

function resumoContatoMensagem(m, limite = 120) {
  const contatos = extrairContatosMensagem(m);
  if (!contatos.length) {
    return ehPlaceholderContato(m?.texto, m?.tipo) ? '👤 Contato compartilhado' : '';
  }

  const primeiro = contatos[0];
  const base = contatos.length > 1
    ? `👥 ${contatos.length} contatos compartilhados`
    : `👤 ${primeiro.nome || 'Contato compartilhado'}${primeiro.telefone ? ` • ${primeiro.telefone}` : ''}`;

  return resumoTextoLivre(base, limite);
}

function resumoUltimaMensagemConversa(c) {
  if (ehPlaceholderContato(c?.ultima_mensagem, c?.ultima_mensagem_tipo)) return '👤 Contato compartilhado';
  const tipo = c?.ultima_mensagem_tipo ? `[${c.ultima_mensagem_tipo}] ` : '';
  return tipo + (c?.ultima_mensagem || '');
}

function detalhesContatoParaCopiar(contato) {
  return [
    contato?.nome ? `Nome: ${contato.nome}` : '',
    contato?.telefone ? `Telefone: ${contato.telefone}` : '',
    contato?.organizacao ? `Organização: ${contato.organizacao}` : ''
  ].filter(Boolean).join('\n');
}

function renderizarContatosMensagem(m) {
  const contatos = extrairContatosMensagem(m);

  if (!contatos.length && ehPlaceholderContato(m?.texto, m?.tipo)) {
    return `
      <div class="contact-message-card contact-message-warning">
        <div class="contact-message-icon">👤</div>
        <div class="contact-message-info">
          <strong>Contato recebido</strong>
          <span>O WhatsApp informou que esta mensagem é um contato.</span>
          <small>Não encontrei os dados do contato no payload salvo. Verifique se a Edge Function está gravando o campo evolution_payload.</small>
        </div>
      </div>
    `;
  }
  ...
  return contatos.map((contato) => {
    const nome = contato.nome || 'Contato compartilhado';
    const telefone = contato.telefone || '';
    const org = contato.organizacao || '';
    const waLink = telefoneContatoParaWaMe(telefone);
    const copy = encodeURIComponent(detalhesContatoParaCopiar(contato));
    ...
    return `
      <div class="contact-message-card">
        ...
        <div class="contact-message-info">
          <strong>${escapeHtml(nome)}</strong>
          ${telefone ? `<span>${escapeHtml(telefone)}</span>` : '<span>Telefone não identificado</span>'}
          ${org ? `<small>${escapeHtml(org)}</small>` : '<small>Contato compartilhado pelo WhatsApp</small>'}
        </div>
        <div class="contact-message-actions">
          ${waLink ? `<a class="contact-action-link" href="${escapeHtml(waLink)}" target="_blank" rel="noopener">Abrir WhatsApp</a>` : ''}
          ${copy ? `<button type="button" class="contact-action-btn" data-copy-contact="${escapeHtml(copy)}">Copiar</button>` : ''}
        </div>
      </div>
    `;
  }).join('');
...
async function copiarContatoRecebido(botao) {
  const valor = botao?.getAttribute('data-copy-contact') || '';
  const texto = (() => {
  ...
  try {
    ...
    toast('Contato copiado para a área de transferência.', 'sucesso');
  } catch {
...
function renderizarMensagem(m) {
  ...
  const contato = renderizarContatosMensagem(m);
  const media = renderizarMedia(m);
  const textoEhSomentePlaceholderContato = mensagemTemContato(m) && ehPlaceholderContato(textoOriginal, m.tipo);
  const texto = textoOriginal && !textoEhSomentePlaceholderContato
    ? `<div class="bubble-text">${renderizarTextoMensagem(textoOriginal, ehInterna)}</div>`
    : '';
...
function resumoMensagemParaMenu(m, limite = 120) {
  const contato = resumoContatoMensagem(m, limite);
...
async function copiarTextoMensagem(id) {
  const msg = mensagensRenderizadas.get(String(id));
  if (!msg) return;
  const contato = resumoContatoMensagem(msg, 400);
  const conteudo = contato || String(msg.texto || msg.media_url || msg.media_nome || '').trim();
...
function carregarMidiaSobDemanda(botao) {
  const url = botao?.getAttribute('data-url') || '';
  const nome = botao?.getAttribute('data-nome') || 'arquivo';
  const tipo = String(botao?.getAttribute('data-tipo') || '').toUpperCase();
  if (!url || !botao?.parentElement) return;
  registrarMonitorMidiaSobDemanda(tipo);

  let html = '';
  if (tipo === 'IMAGEM') {
    html = `<img class="media-img" src="${escapeHtml(url)}" alt="${escapeHtml(nome)}" loading="lazy" data-open-media="${escapeHtml(url)}">`;
  } else if (tipo === 'AUDIO') {
    html = `<audio class="media-audio" controls preload="none" src="${escapeHtml(url)}"></audio>`;
  } else if (tipo === 'VIDEO') {
    html = `<video class="media-video" controls preload="none" src="${escapeHtml(url)}"></video>`;
  } else {
  ...
  if (novo?.dataset?.openMedia) {
    novo.addEventListener('click', () => window.open(novo.dataset.openMedia, '_blank'));
  }
}
...
function renderizarMedia(m) {
  ...
  if (tipo === 'DOCUMENTO') {
    return `<a class="doc-link" href="${url}" target="_blank" rel="noopener">📄 ${nome}</a>`;
  }

  const rotulos = {
    IMAGEM: '🖼️ Carregar imagem',
    AUDIO: '▶️ Carregar áudio',
    VIDEO: '🎬 Carregar vídeo'
  };

  return `<button type="button" class="media-lazy-card" data-load-media="1" data-url="${url}" data-nome="${nome}" data-tipo="${escapeHtml(tipo)}">
    <span>${rotulos[tipo] || '📎 Carregar mídia'}</span>
    <small>${nome}</small>
    <em>Toque para baixar somente quando precisar.</em>
  </button>`;
}
...
function limiteArquivoPorTipo(tipo) {
  return MEDIA_LIMITES_BYTES[String(tipo || '').toUpperCase()] || MEDIA_LIMITES_BYTES.DOCUMENTO;
}

function alterarExtensaoArquivo(nome, novaExtensao) {
  const base = String(nome || 'imagem').replace(/\\.[^.]+$/, '') || 'imagem';
  return `${base}${novaExtensao}`;
}

async function comprimirImagemParaEgress(file) {
  if (!file || !String(file.type || '').startsWith('image/')) return file;
  if (/gif|svg/i.test(file.type || '') || file.size < IMAGEM_COMPRIMIR_ACIMA_DE) return file;

  const objectUrl = URL.createObjectURL(file);
  try {
    const img = new Image();
    img.decoding = 'async';
    const carregada = new Promise((resolve, reject) => {
      img.onload = resolve;
      img.onerror = reject;
    });
    img.src = objectUrl;
    ...
    const maiorLado = Math.max(img.naturalWidth || img.width, img.naturalHeight || img.height);
    const escala = maiorLado > IMAGEM_MAX_LADO ? IMAGEM_MAX_LADO / maiorLado : 1;
    ...
    const blob = await new Promise((resolve) => canvas.toBlob(resolve, 'image/jpeg', IMAGEM_QUALIDADE));
    if (!blob || blob.size >= file.size) return file;

    return new File([blob], alterarExtensaoArquivo(file.name, '.jpg'), {
      type: 'image/jpeg',
      lastModified: Date.now()
    });
  } catch (erro) {
    console.warn('Não foi possível otimizar a imagem antes do envio:', erro?.message || erro);
    return file;
  } finally {
...
async function prepararArquivoEconomico(file) {
  const tipo = detectarTipoArquivo(file);
  const limite = limiteArquivoPorTipo(tipo);

  if (file.size > limite) {
    throw new Error(`Arquivo muito grande para o modo econômico. Limite para ${tipo}: ${formatarTamanho(limite)}.`);
  }

  if (tipo === 'IMAGEM') {
    return await comprimirImagemParaEgress(file);
  }
...
async function mostrarPreviewArquivo(file) {
  ...
  const limite = limiteArquivoPorTipo(tipo);
  if (file.size > limite) {
    alert(`Arquivo muito grande. Para economizar Supabase, use até ${formatarTamanho(limite)} para ${tipo}.`);
...
async function enviarAnexoSelecionado() {
  ...
  const arquivoOriginal = arquivoSelecionado;
  const file = await prepararArquivoEconomico(arquivoOriginal);
  const tipo = detectarTipoArquivo(file);
  const base64 = await arquivoParaBase64(file);

  if (file.size < arquivoOriginal.size) {
    console.info(`Imagem otimizada para reduzir Supabase Egress/Storage: ${formatarTamanho(arquivoOriginal.size)} → ${formatarTamanho(file.size)}`);
  }
  ...
  registrarMonitorUpload(file, arquivoOriginal);
...
function agendarCarregarConversas(delay = DEBOUNCE_REALTIME_CONVERSAS_MS) {
  if (refreshConversasTimer) clearTimeout(refreshConversasTimer);
  refreshConversasTimer = setTimeout(async () => {
    refreshConversasTimer = null;
    await carregarConversas({ silencioso: true });
  }, delay);
}

function agendarCarregarMensagens(conversaId, opcoes = {}, delay = DEBOUNCE_REALTIME_MENSAGENS_MS) {
  if (!conversaId) return;
  if (refreshMensagensTimer) clearTimeout(refreshMensagensTimer);
  refreshMensagensTimer = setTimeout(async () => {
    refreshMensagensTimer = null;
    if (conversaAtual?.id === conversaId) {
      await carregarMensagensENotas(conversaId, opcoes);
    }
  }, delay);
}
  ...
  realtimeChannel = db.channel('medplus-chat-realtime')
    ...
    .on('postgres_changes', { event: '*', schema: 'public', table: 'mensagens' }, (payload) => {
      if (conversaAtual && (payload.new?.conversa_id === conversaAtual.id || payload.old?.conversa_id === conversaAtual.id)) {
        agendarCarregarMensagens(conversaAtual.id, { force: true });
      }
      agendarCarregarConversas();
      ...
      if (payload.eventType === 'INSERT' && payload.new?.remetente_tipo === 'CLIENTE') {
        ...
        const texto = resumoMensagemParaMenu(payload.new, 110) || resumoUltimaMensagemConversa(c) || 'Nova mensagem';
    ...
    .on('postgres_changes', { event: '*', schema: 'public', table: 'notas_internas' }, (payload) => {
      if (conversaAtual && (payload.new?.conversa_id === conversaAtual.id || payload.old?.conversa_id === conversaAtual.id)) {
        agendarCarregarMensagens(conversaAtual.id, { force: true });
      }

      if (payload.eventType === 'INSERT') {
        notificarNotaInternaMencionada(payload.new);
      }
    })
    .on('postgres_changes', { event: '*', schema: 'public', table: 'transferencias' }, (payload) => {
      agendarCarregarConversas();
      if (payload.eventType === 'INSERT') {
        notificarTransferenciaParaMim(payload.new);
      }
      if (conversaAtual) agendarCarregarMensagens(conversaAtual.id);
    })
    .subscribe((status) => {
      console.log('Realtime:', status);
      realtimeConectado = status === 'SUBSCRIBED';
      if (status === 'CHANNEL_ERROR' || status === 'TIMED_OUT' || status === 'CLOSED') {
        console.warn('Realtime indisponível. O sistema continuará atualizando em modo econômico por polling.');
      }
    });

  // Antes era a cada 10s. Agora roda em intervalo maior apenas como rede de segurança,
  // porque o Realtime já recebe as mudanças sem ficar relendo tudo o tempo todo.
...
async function carregarClientesAdmin() {
  const { data, error } = await db.from('clientes').select(CLIENTES_SELECT).order('criado_em', { ascending: false }).limit(500);
  if (error) {
...
function contextoMensagemRapidaCampo(inputId) {
  const input = $(inputId);
  if (!input) return null;
...
function contextoMensagemRapida() {
  return contextoMensagemRapidaCampo('messageInput');
}

function contextoMensagemRapidaNota() {
  return contextoMensagemRapidaCampo('notaInternaInput');
}
...
function renderizarMensagensRapidasNotaDropdown() {
  const box = $('notaQuickRepliesBox');
  const ctx = contextoMensagemRapidaNota();
  ...
  notaQuickReplyContext = ctx;

  if (!mensagensRapidas.length && !mensagensRapidasErro && !tentativaLazyRapidas) {
    tentativaLazyRapidas = true;
    carregarMensagensRapidas(false).then(() => {
      tentativaLazyRapidas = false;
      renderizarMensagensRapidasNotaDropdown();
    });
  }

  const lista = filtrarMensagensRapidas(ctx.termo);
  notaQuickReplyIndex = Math.max(0, Math.min(notaQuickReplyIndex, Math.max(0, lista.length - 1)));

  if (mensagensRapidasErro) {
    box.innerHTML = `
      <div class="quick-replies-title"><span>Mensagens rápidas</span><span>Erro</span></div>
      <div class="quick-replies-empty">Não consegui carregar as mensagens rápidas. Verifique o cadastro em Mensagens rápidas.</div>
    `;
    box.classList.remove('hidden');
    return;
  }
  ...
  if (!lista.length) {
    box.innerHTML = `
      <div class="quick-replies-title"><span>Mensagens rápidas</span><span>Digite /</span></div>
      <div class="quick-replies-empty">Nenhuma mensagem rápida encontrada para esta busca.</div>
    `;
    box.classList.remove('hidden');
    return;
  }
  ...
  box.innerHTML = `
    <div class="quick-replies-title"><span>Mensagens rápidas para nota</span><span>Enter para inserir</span></div>
    ${lista.map((r, index) => `
      <button type="button" class="quick-reply-item ${index === notaQuickReplyIndex ? 'active' : ''}" data-note-quick-id="${escapeHtml(r.id)}" role="option" aria-selected="${index === notaQuickReplyIndex ? 'true' : 'false'}">
        <span class="quick-reply-head">
          <span class="quick-reply-title">${escapeHtml(r.titulo || r.atalho)}</span>
          <span class="quick-reply-shortcut">/${escapeHtml(r.atalho || '')}</span>
        </span>
  ...
  box.querySelectorAll('[data-note-quick-id]').forEach((btn) => {
    btn.addEventListener('mousedown', (e) => e.preventDefault());
    btn.addEventListener('click', () => {
      const item = mensagensRapidas.find((r) => r.id === btn.dataset.noteQuickId);
      inserirMensagemRapidaNota(item);
    });
  });
}

function esconderMensagensRapidasNota() {
  const box = $('notaQuickRepliesBox');
  if (box) {
    box.classList.add('hidden');
    box.innerHTML = '';
  }
  notaQuickReplyContext = null;
  notaQuickReplyIndex = 0;
}

function inserirMensagemRapidaNota(item) {
  if (!item) return false;
  const input = $('notaInternaInput');
  const ctx = notaQuickReplyContext || contextoMensagemRapidaNota();
...
function moverSelecaoMensagemRapidaNota(delta) {
  const ctx = contextoMensagemRapidaNota();
  if (!ctx) return false;
  const lista = filtrarMensagensRapidas(ctx.termo);
  if (!lista.length) return false;
  notaQuickReplyIndex = (notaQuickReplyIndex + delta + lista.length) % lista.length;
  renderizarMensagensRapidasNotaDropdown();
  return true;
}

function selecionarMensagemRapidaNotaAtual() {
  const ctx = contextoMensagemRapidaNota();
  if (!ctx) return false;
  const lista = filtrarMensagensRapidas(ctx.termo);
  const item = lista[notaQuickReplyIndex] || lista[0];
  if (!item) return false;
  return inserirMensagemRapidaNota(item);
}

function atualizarSugestoesNotaInterna() {
  const temRapida = !!contextoMensagemRapidaNota();
  const temMencao = !!obterContextoMencaoNota();
...
function configurarEventos() {
  ...
  $('btnToggleMonitorDados')?.addEventListener('click', alternarMonitorDados);
  $('btnSalvarMonitorConfig')?.addEventListener('click', salvarConfiguracaoMonitorSupabase);
  $('btnLimparMonitorManual')?.addEventListener('click', limparValoresManuaisMonitorSupabase);
  $('btnResetMonitorDados')?.addEventListener('click', resetarMonitorSupabase);
...
/* ==========================================================
   APP UI PREMIUM — navegação mobile tipo aplicativo
   Camada não destrutiva: preserva Supabase/Evolution e usa
   as funções existentes do sistema quando disponíveis.
   ========================================================== */
(function configurarAppUiPremium() {
  ...
  function abrirAbaKanban() {
    if (typeof mostrarKanban === 'function') {
      mostrarKanban();
    }
    byId('appScreen')?.classList.add('kanban-mode');
    byId('appScreen')?.classList.remove('mobile-chat-open');
    setTab('kanban');
  }

  function abrirAbaChat() {
    const app = byId('appScreen');
    if (typeof mostrarChat === 'function' && typeof conversaAtual !== 'undefined' && conversaAtual) {
      mostrarChat();
      app?.classList.add('mobile-chat-open');
    } else {
      app?.classList.remove('kanban-mode');
      app?.classList.remove('mobile-chat-open');
      byId('kanbanPanel')?.classList.add('hidden');
      byId('messagesBox')?.classList.remove('hidden');
      document.querySelector('.composer')?.classList.remove('hidden');
    }
    setTab('chat');
  }

  function abrirAbaAdmin() {
    setTab('admin');
    if (typeof abrirAdminModal === 'function') {
      abrirAdminModal('clientes');
      return;
    }
    const primeiroAdmin = byId('btnAdminClientes') || byId('btnAdminUsuarios');
    primeiroAdmin?.click();
  }
  ...
  function sairApp() {
    ...
    byId('btnLogout')?.click();
  }

  function sincronizarPeloEstado() {
    const app = byId('appScreen');
    if (!app || app.classList.contains('hidden')) return;
    if (app.classList.contains('kanban-mode')) setTab('kanban');
    else setTab('chat');
  }

  document.addEventListener('DOMContentLoaded', () => {
    byId('btnAppNavKanban')?.addEventListener('click', abrirAbaKanban);
    byId('btnAppNavChat')?.addEventListener('click', abrirAbaChat);
    byId('btnAppNavAdmin')?.addEventListener('click', abrirAbaAdmin);
    byId('btnAppNavSair')?.addEventListener('click', sairApp);

    const voltar = byId('btnMobileBack');
    voltar?.addEventListener('click', () => {
      byId('appScreen')?.classList.remove('kanban-mode');
      setTab('chat');
    });

    const observar = byId('appScreen');
    if (observar && 'MutationObserver' in window) {
      new MutationObserver(sincronizarPeloEstado).observe(observar, { attributes: true, attributeFilter: ['class'] });
    }

    setTimeout(sincronizarPeloEstado, 250);
  });
})();
